...
void odomCalculate(){
  ...
  radius = (encoder1data-encoder_pre)/angle;
  encoder_pre = encoder1data;
  if(angle>=0){
    odom_data.y += radius - radius*cos(angle);
...
{ 
  ...
  //ros::Timer timer100ms=n.createTimer(ros::Duration(0.1),timerCallback);
